...
implements Comparable
{
	...
	private void initRate() {
		for (Iterator itor = this.person.getBooks().iterator(); itor.hasNext();) {
			Book book = (Book) itor.next();
			AssignVo vo = this.map.get(book.getSubCategory().getId());
			if (vo != null) {
				AssignBookVo bookVo = new AssignBookVo(book, Integer.valueOf(vo.getRemainQuantity()));
				this.bookList.add(bookVo);
				if (book.getSubCategory().getId().intValue() != this.subCategoryId)
					this.rate += this.map.get(book.getSubCategory().getId()).getRate();
				else {
 ...
 * mfw.acegi.vo.PersonAssignVo JD-Core Version: 0.6.2
 */